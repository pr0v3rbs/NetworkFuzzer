    ...
    )
{
    ...
    struct PacketInfo*          packetInfo = NULL;
    ...
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode)
    {
        case IOCTL_FILTER_SETTING:
            ...
            break;
        case IOCTL_FILTER_GET_PACKET:
            if (PeekPacket(&packetInfo, FALSE))
            {
                if (IrpSp->Parameters.DeviceIoControl.OutputBufferLength >= packetInfo->packetSize + sizeof(ULONG))
                {
                    NdisMoveMemory(Irp->AssociatedIrp.SystemBuffer, &packetInfo->frameSize, sizeof(ULONG));
                    CopyPacketFromNetBufferLists((BYTE*)Irp->AssociatedIrp.SystemBuffer + sizeof(ULONG),
                        packetInfo->NetBufferLists,
                        packetInfo->frameSize,
                        packetInfo->packetSize,
                        packetInfo->direction);
                    InfoLength = packetInfo->packetSize + sizeof(ULONG);
                    Status = STATUS_SUCCESS;
                }
                else
                {
            ...
            break;
        case IOCTL_FILTER_SET_MUTATED_PACKET:
            if (PopPacket(&packetInfo, FALSE))
            {
                if (packetInfo->packetSize >= IrpSp->Parameters.DeviceIoControl.InputBufferLength)
                {
                    CopyPacketFromBuffer(packetInfo->NetBufferLists,
                        Irp->AssociatedIrp.SystemBuffer,
                        packetInfo->frameSize,
                        packetInfo->packetSize,
                        packetInfo->direction);
                }
                DeliverPacket(packetInfo);
            ...
            break;
        case IOCTL_FILTER_SET_PACKET_UNMUTATED:
            if (PopPacket(&packetInfo, FALSE))
            {
                DeliverPacket(packetInfo);
                RemovePacket(packetInfo);
