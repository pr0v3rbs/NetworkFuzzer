...
--*/
{
    ...
    /*if (NetBufferLists->NblFlags & 0x10000000)
    {
        PUCHAR  packet = NULL;
        ...
        *(packet + ((struct TCP*)packet)->th_off * 4) = 'G';
    }*/
...
--*/
{
    ...
    do
    {
        ...
        if (gFilterEnable)
        {
            ...
            frame = NetBufferLists->FirstNetBuffer->MdlChain->MappedSystemVa;
            frame += NetBufferLists->FirstNetBuffer->DataOffset;

            if (IsIpv4((struct ETH*)frame) && IsLocalnetworkPacket(CONVERT_IP(frame)))
            {
                //BOOLEAN DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
                ULONG frameSize = 0;

                for (PNET_BUFFER netBuffer = NET_BUFFER_LIST_FIRST_NB(NetBufferLists);
                    netBuffer != NULL;
                    netBuffer = NET_BUFFER_NEXT_NB(netBuffer))
                {
                    packetSize += NET_BUFFER_DATA_LENGTH(netBuffer);
                }

                if (IsTargetPacket(frame, &frameSize) &&
                    packetSize > frameSize &&
                    IsLocalnetworkPacket(CONVERT_IP(frame)))
                {
                    //InsertTailPacket(pFilter->FilterHandle, frameSize, packetSize, NetBufferLists, PortNumber, 0, SendFlags, DispatchLevel, send);
                    //isPended = TRUE;
                    Mutate(NetBufferLists, packetSize, frameSize, send);
                    //MutateFrame(NetBufferLists, frameSize, send);
                }
            }
        }
...
--*/
{
    ...
    do
    {
        ...
        if (gFilterEnable)
        {
            ...
            if (IsIpv4((struct ETH*)frame))
            {
                //BOOLEAN DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(ReceiveFlags);
                ULONG frameSize = 0;
                ...
                if (IsTargetPacket(frame, &frameSize) &&
                    ...
                    IsLocalnetworkPacket(CONVERT_IP(frame))*/)
                {
                    //InsertTailPacket(pFilter->FilterHandle, frameSize, packetSize, NetBufferLists, PortNumber, NumberOfNetBufferLists, ReceiveFlags, DispatchLevel, receive);
                    //isPended = TRUE;
                    Mutate(NetBufferLists, packetSize, frameSize, receive);
                    //MutateFrame(NetBufferLists, frameSize, receive);
                }
            }
        }
